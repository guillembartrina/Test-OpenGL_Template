...
void Drawable::draw() const
{
	if (loaded)
	{
		...
		std::cerr << "Drawing drawable while not loaded" << std::endl;
}
...
void Drawable::getMM_FromModel(Model *m, glm::vec3 &min, glm::vec3 &max)
{
	...
	{
		if (m->vertices()[i] < min.x)
			min.x = m->vertices()[i];
		if (m->vertices()[i] > max.x)
			max.x = m->vertices()[i];
		if (m->vertices()[i + 1] < min.y)
			min.y = m->vertices()[i + 1];
		if (m->vertices()[i + 1] > max.y)
			max.y = m->vertices()[i + 1];
		if (m->vertices()[i + 2] < min.z)
			min.z = m->vertices()[i + 2];
		if (m->vertices()[i + 2] > max.z)
			max.z = m->vertices()[i + 2];
	}
}